...
public class upload extends AppCompatActivity {
    ...
    //method ketika image dipilih
    public void setfoto(View view) {
        Intent pickImage = new Intent(Intent.ACTION_PICK); //memberikan fungsi intent
        pickImage.setType("image/*"); //memberikan request secara fungsional
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode==RESULT_OK){
            if(requestCode==SELECT_PICTURE){
                ...
                try {
                    //mengubah data menjadi inputstream yang diolah menjadi bitmap dan ditempatkan pada imageview
                    InputStream stream = getContentResolver().openInputStream(imageUri);
                    Bitmap gambar = BitmapFactory.decodeStream(stream); //menyimpan gambar ke dalam tipe bitmap
                    gambardiupload.setImageBitmap(gambar); //mengatur gambar untuk ditampilkan
                } catch (FileNotFoundException e) {
                    Log.w("FileNotFoundException", e.getMessage());
                    Toast.makeText(this, "Unable to load image", Toast.LENGTH_SHORT).show(); //menampilkan pesan toast
                }
            }

            //Ketika user tidak memilih foto
        }else{
            Toast.makeText(this, "Picture not selected", Toast.LENGTH_SHORT).show(); //menampilkan pesan toast
        }
    }
    public void uploadingambar(View view) { //method ketika image dipilih
        ...
        //menimpan gambar ke dalam tipe bitmap
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);
        //melakukan kompress gambar
        ...
        task.addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                String image = taskSnapshot.getDownloadUrl().toString(); //menyimpan gambar kedalam variabel image dengan tipe string
                dbasePost user = new dbasePost(caption.getText().toString(), image, title.getText().toString(), idCurrentUser);
                //meyimpan gambar ke dalam kode database
            ...
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(upload.this, "Upload Failure!", Toast.LENGTH_SHORT).show(); //menampilkan pesan toast
                dlg.dismiss();//dialog ditutup
            }
        });
